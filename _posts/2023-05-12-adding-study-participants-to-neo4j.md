...
Fig 1. Architecture diagram of Trellis resources involved in adding study information to the Trellis metadata store.
...
Fig 2. Sequence diagram describing how study participants will be added to the Trellis metadata store.
...
Fig 3. Diagram describing the new graph model for connecting people to studies in the Trellis metadata store.
...
Because all of the relationships between people and studies are tracked in the same graph database, we can use graph traversal to find samples that belong to multiple studies.
