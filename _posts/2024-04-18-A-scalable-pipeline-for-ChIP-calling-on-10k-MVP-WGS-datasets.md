...
Post running Mutect2, a filtering step was implemented to refine the identified variants. This step utilized a custom script executed on a VM provisioned within the same GCP project. The script leverages `gsutil` to download the VCF files generated by Mutect2 from a designated GCS bucket. A temporary directory (/tmp/vcf_filtering) was created on the VM to store the downloaded VCFs and the filtered outputs.
...
gsutil -q -m cp "$TEMP_DIR"/*.vcf.gz* "$BUCKET_PATH"
...
- Re-run Mutect2 for these 6 samples alone - If they run successfully, then we have to redo the filtering, indexing, merging and normalization steps to include these files with the 10k results. 
